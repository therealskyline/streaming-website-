...
def main():
    """
    Script principal pour démarrer l'application web AnimeStream
    ...
    print(f"Démarrage de l'application AnimeStream sur le port {port}")
    print(f"Répertoire courant: {os.getcwd()}")
    
    # Chemin vers l'application Flask
    app_path = os.path.join("realweb", "final_website", "app.py")
    ...
    if not os.path.isdir(app_dir):
        print(f"ERREUR: Le dossier {app_dir} n'existe pas")
        sys.exit(1)
    else:
        print(f"Le dossier {app_dir} existe!")
        print(f"Contenu du dossier: {os.listdir(app_dir)}")
